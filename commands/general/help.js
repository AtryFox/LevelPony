    ...
    async run(msg, args) { // eslint-disable-line complexity
        ...
        if (args.command && !showAll) {
            if (commands.length === 1) {
				...
				`;
                if (commands[0].aliases.length > 0) description += `\n**Aliases:** ${commands[0].aliases.join(', ')}`;
                description += `\n${oneLine`
					**Group:** ${commands[0].group.name}
					(\`${commands[0].groupID}:${commands[0].memberName}\`)
				`}`;
                if (commands[0].details) description += `\n**Details:** ${commands[0].details}`;
                if (commands[0].examples) description += `\n**Examples:**\n\`\`\`${commands[0].examples.join('\n')}\`\`\``;

                let embed = new MessageEmbed({
                    title: `**__Command \`${commands[0].name}\`:__**`,
                ...
                try {
                    ...
                    if (msg.channel.type !== 'dm') {
                        const response = await msg.reply('Sent you a DM with information.');
                    ...
                    messages.push(await msg.reply({embed: PonyUtils.generateFailEmbed(msg, 'Unable to send you the help DM. You probably have DMs disabled.')}));
                }
                return messages;
            } else if (commands.length > 1) {
                return msg.reply({embed: PonyUtils.generateFailEmbed(msg, 'Multiple commands found. Please be more specific.')});
            } else {
                return msg.reply({
                    embed: PonyUtils.generateFailEmbed(msg, 'Unable to identify command.',
                        `Use ${msg.usage(
